...
const faqData = [
  { q: "What is PiCipher?", a: "A fully decentralized visual decryption game with voice input mechanics." },
  { q: "How do I play?", a: "Connect your wallet, analyze the 4 images, guess the shared concept using your microphone, and earn Celo bounties." },
...
export default function FAQ() {
  ...
  return (
    <div data-component-id="e1c7b158" className="min-h-screen bg-black text-white p-4 sm:p-6 md:p-12 font-mono selection:bg-[#35D07F] selection:text-black">
      <div data-theme-role="primary-surface" className="max-w-4xl mx-auto border-l-2 border-r-2 border-[#35D07F]/30 min-h-[80vh] p-4 sm:p-8 flex flex-col relative shadow-[0_0_30px_rgba(53,208,127,0.05)]">
        {/* Decorative corner accents */}
        <div data-theme-role="primary-surface" className="absolute top-0 left-0 w-4 h-4 border-t-2 border-l-2 border-[#35D07F]" />
        <div data-testid="text-6e087e" className="absolute top-0 right-0 w-4 h-4 border-t-2 border-r-2 border-[#35D07F]" />
        <div data-testid="text-b6aa36" className="absolute bottom-0 left-0 w-4 h-4 border-b-2 border-l-2 border-[#35D07F]" />
        <div data-component-id="547ceaf9" className="absolute bottom-0 right-0 w-4 h-4 border-b-2 border-r-2 border-[#35D07F]" />

        <div data-theme-role="primary-surface" className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-12 gap-4">
          ...
          {faqData.map((item, i) => (
            <div key={i} onClick={() => toggle(i)} className="border border-white/10 p-5 cursor-pointer bg-white/5 hover:bg-white/10 transition-colors group">
              ...
              <div className={`overflow-hidden transition-all duration-300 ${activeIndex === i ? 'max-h-40 mt-4 opacity-100 border-t border-white/10 pt-4' : 'max-h-0 opacity-0'}`}>
                <p data-cy="cy-d06f71" className="text-neutral-400 text-sm sm:text-base leading-relaxed font-sans">{item.a}</p>
              </div>
            </div>
          ))}
        </div>
        <div data-testid="text-f8fa25" className="mt-12 text-center text-xs text-neutral-600 tracking-widest">
