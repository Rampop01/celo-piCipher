...
export default function VictoryScreen({ profile, networkName }) {
  ...
  const handleShare = () => {
    ...
    const url = "https://picipher.com"; 
    const twitterUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent(text)}&url=${encodeURIComponent(url)}`;
    window.open(twitterUrl, "_blank");
  };

  return (
    <div data-cy="cy-c9fff3" className="fixed inset-0 z-[100] bg-black text-white flex flex-col items-center justify-center p-6 overflow-hidden">
      {/* Background Grid - Celo Theme */}
      <div data-theme-role="primary-surface" className="absolute inset-0 bg-[linear-gradient(rgba(18,16,16,0)_50%,rgba(0,0,0,0.25)_50%),linear-gradient(90deg,rgba(53,208,127,0.06),rgba(0,0,0,0.02))] bg-[length:100%_4px,4px_100%] opacity-20 pointer-events-none"></div>

      {/* Main Content */}
      <div data-testid="text-c8c569" className="relative z-10 flex flex-col items-center max-w-2xl text-center">
        <div data-theme-role="primary-surface" className="relative mb-8 group">
          <div data-testid="text-f58333" className="absolute inset-0 bg-[#35D07F] blur-[50px] opacity-20 group-hover:opacity-40 transition-opacity"></div>
        ...
        <div data-theme-role="primary-surface" className="bg-black/50 border border-[#35D07F]/30 p-6 backdrop-blur w-full mb-8 shadow-[0_0_20px_rgba(53,208,127,0.1)]">
          <p data-testid="text-86af96" className="text-neutral-400 font-mono text-sm md:text-base leading-relaxed mb-4">
            Congratulations, <span data-cy="cy-bf7401" className="text-white font-bold">{profile?.nickname || "Hacker"}</span>. 
          ...
          <div data-testid="text-653658" className="flex justify-between border-t border-neutral-800 pt-4 text-xs font-mono">
             <span data-theme-role="primary-surface" className="text-neutral-500">FINAL CLASSIFICATION:</span>
